...
async function seed() {
  ...
  try {
    ...
    `, [igorHash, nextraHash]);
    await client.query(`SELECT setval(pg_get_serial_sequence('users','id'), 20, false)`);
    ...
    `);
    await client.query(`SELECT setval(pg_get_serial_sequence('clients','id'), 20, false)`);
    ...
    await client.query(`
      INSERT INTO tickets (id, business_unit_id, client_id, client_name, created_by_user_id, channel,
        occurrence_type, criticality, area_responsible, description, status, sla_state, sla_deadline, revenue_at_risk, is_vip, approval_status, reopen_count, nf_number) VALUES
        ('CSO-0001','led',1,'CRIALED Integrações',3,'whatsapp','technical_support','high','support','Painel P3 sem sinal após atualização de firmware do receiver MRV300.','in_triage','warn',NOW()+INTERVAL '4h',25000,TRUE,'pending',0,'NF-2026-0001'),
        ('CSO-0002','led',2,'LEDCOLOR Soluções',2,'email','quality_issue','critical','cso','Lote inteiro com pixel cluster morto — 40 módulos comprometidos.','in_resolution','overdue',NOW()-INTERVAL '2h',60000,FALSE,'pending',0,'NF-2026-0002'),
        ('CSO-0003','led',3,'DUOVISION Sistemas',3,'whatsapp','delivery_issue','medium','warehouse','NF com valor divergente — trava fiscal.','closed','done',NOW()+INTERVAL '24h',0,FALSE,'approved',0,'NF-2026-0003'),
        ('CSO-0004','led',4,'SEVEN Events',3,'phone','technical_support','high','support','Controladora Nova Star desligando durante evento ao vivo.','new','ok',NOW()+INTERVAL '8h',35000,FALSE,'pending',0,'NF-2026-0004'),
        ('CSO-0005','led',5,'Igreja Renascer SP',7,'whatsapp','delivery_issue','medium','purchasing','Prazo estourado 12 dias. Obra parada.','awaiting_supplier','warn',NOW()+INTERVAL '2h',22000,FALSE,'pending',0,'NF-2026-0005'),
        ('CSO-0006','led',8,'VISION TECH Pro',2,'email','quality_issue','critical','support','Sistema inoperante. Cliente VIP SLA nível 1. Risco de rescisão.','in_technical_analysis','overdue',NOW()-INTERVAL '6h',200000,TRUE,'pending',2,'NF-2026-0006'),
        ('CSO-0007','led',2,'LEDCOLOR Soluções',4,'whatsapp','rma','high','rma','Troca de módulos com defeito de fabricação. 15 unidades.','awaiting_info','warn',NOW()+INTERVAL '3h',40000,FALSE,'pending',0,'NF-2026-0007'),
        ('CSO-0008','led',7,'SEAL Indoor',3,'email','technical_support','low','support','Configuração de brilho automático.','awaiting_client_validation','ok',NOW()+INTERVAL '48h',0,FALSE,'approved',0,'NF-2026-0008'),
        ('CSO-0009','led',1,'CRIALED Integrações',4,'email','rma','low','rma','RMA de módulo danificado no transporte.','closed','done',NOW()+INTERVAL '72h',0,FALSE,'approved',0,'NF-2026-0009'),
        ('CSO-0010','led',8,'VISION TECH Pro',2,'whatsapp','technical_support','critical','support','Problema reincidente após reparo. Terceira ocorrência.','reopened','overdue',NOW()-INTERVAL '12h',80000,TRUE,'pending',3,'NF-2026-0010')
    ...
    `);
    await client.query(`SELECT setval(pg_get_serial_sequence('complaints','id'), 20, false)`);
    ...
    await client.query(`SELECT setval(pg_get_serial_sequence('returns','id'), 20, false)`);
    ...
    await client.query(`SELECT setval(pg_get_serial_sequence('rma','id'), 20, false)`);
    ...
    await client.query(`
      ...
      ON CONFLICT (id) DO UPDATE SET status=EXCLUDED.status
    `);
    await client.query(`SELECT setval(pg_get_serial_sequence('recovery_plans','id'), 10, false)`);
